...
include ('includes/adminhead.php');
  ...
  if ($comp_pass != $old_pass) {
    ?> <script>
      swal({
        ...
        window.location="adminprofile.php#password_heading";
        })
      </script>
    <?php
  }else {
  ...
  $adminpassword = "UPDATE staff set password = '$new_pass' WHERE username = '$user_id'";

  if (mysqli_query ($db, $adminpassword)){
    ...
    echo "failed to run $adminpassword  " . mysqli_error($db);
    ?><script>
        swal(
