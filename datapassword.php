...
include ('includes/datahead.php');
  ...
  if ($comp_pass != $old_pass) {
    ?> <script>
      swal({
        ...
        window.location="dataprofile.php#password_heading";
        })
      </script>
    <?php
  }else {
  ...
  $datapassword = "UPDATE staff set password = '$new_pass' WHERE username = '$user_id'";

  if (mysqli_query ($db, $datapassword)){
    ...
    echo "failed to run $datapassword  " . mysqli_error($db);
    ?><script>
      swal(
